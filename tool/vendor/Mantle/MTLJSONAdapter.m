...
- (id)initWithModelClass:(Class)modelClass {
	...
	NSSet *propertyKeys = [self.modelClass mtl_propertyKeys];
...
- (NSDictionary *)JSONDictionaryFromModel:(id<MTLJSONSerializing>)model error:(NSError **)error {
	...
	[dictionaryValue enumerateKeysAndObjectsUsingBlock:^(NSString *propertyKey, id value, BOOL *stop) {
		...
		void (^createComponents)(id, NSString *) = ^(NSObject *obj, NSString *keyPath) {
...
- (id)modelFromJSONDictionary:(NSDictionary *)JSONDictionary error:(NSError **)error {
	...
	for (NSString *propertyKey in [self.modelClass mtl_propertyKeys]) {
		id JSONKeyPaths = self.JSONKeyPathsByPropertyKey[propertyKey];
...
+ (NSDictionary *)valueTransformersForModelClass:(Class)modelClass {
	...
	for (NSString *key in [modelClass mtl_propertyKeys]) {
