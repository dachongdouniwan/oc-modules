...
static BOOL MTLValidateAndSetValue(id obj, NSString *key, id value, BOOL forceUpdate, NSError **error) {
	...
	@try {
		...
		if (forceUpdate || value != validatedValue) {
			[(NSObject *)obj setValue:validatedValue forKey:key];
		}
...
+ (void)generateAndCacheStorageBehaviors {
	...
	for (NSString *propertyKey in self.mtl_propertyKeys) {
...
- (void)mergeValuesForKeysFromModel:(id<MTLModel>)model {
	NSSet *propertyKeys = model.class.mtl_propertyKeys;

	for (NSString *key in self.class.mtl_propertyKeys) {
...
- (NSString *)description {
     ...
     *  fallenink
     *  
     *  bugfix: 解决po object对象时，显示"\Uxxxx"，而不是中文的问题
     */
    if ([NSJSONSerialization isValidJSONObject:permanentProperties]) {
        NSData *propertyData = [NSJSONSerialization dataWithJSONObject:permanentProperties options:NSJSONWritingPrettyPrinted error:nil];
        NSString *properties = [[NSString alloc] initWithData:propertyData encoding:NSUTF8StringEncoding];
...
- (void)setValue:(id)value forUndefinedKey:(NSString *)key {
    NSLog(@"[%@] warning: setValue:%@ forUndefinedKey:%@", NSStringFromSelector(_cmd)
          ,value, key);
}
